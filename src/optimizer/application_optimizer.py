...
class ApplicationOptimizer:
    def __init__(self, Appworkflow: ApplicationPerformanceModeling, generate_perf_profile=True, mem_list=None):
    ...
    def generate_perf_profile(self):
        ...
        nx.set_node_attributes(self.App.workflow_graph, {}, 'perf_profile')
        for node in node_list:
            # self.App.workflow_graph.nodes[node]['perf_profile'] = self.appgenerator.gen_rt_mem_data(node)
            pass
    ...
    def update_App_workflow_mem_rt(self, App, mem_dict):
        self.update_mem_rt(App.workflow_graph, mem_dict)
        App.update_rt()
    ...
    def get_perf_cost_table(self, file, start_iterations=1, end_iterations=None):
        ...
        all_available_mem_list = [
            sorted(self.App.workflow_graph.nodes[node]['perf_profile'].keys())
            for node in node_list
        ]
    ...
    def get_optimization_boundary(self):
        node_list = [item for item in self.App.workflow_graph.nodes]
        minimal_mem_configuration = {node: min(self.App.workflow_graph.nodes[node]['perf_profile'].keys()) for node in
                                     node_list}
        maximal_mem_configuration = {node: max(self.App.workflow_graph.nodes[node]['perf_profile'].keys()) for node in
                                     node_list}
        self.App.update_ne()
    ...
    def update_BCR(self):
        node_list = [item for item in self.App.workflow_graph.nodes]
        for node in node_list:
            available_mem_list = [item for item in np.sort(list(self.App.workflow_graph.nodes[node]['perf_profile'].keys()))]
            available_rt_list = [self.App.workflow_graph.nodes[node]['perf_profile'][item] for item in available_mem_list]
            ...
            self.App.workflow_graph.nodes[node]['BCR'] = np.abs(slope)
        ...
        for node in node_list:
            if (BCR):
                available_mem_list = [item for item in
                                      np.sort(list(self.App.workflow_graph.nodes[node]['perf_profile'].keys()))]
                ...
                if (BCRinverse):
                    available_mem_list = [item for item in mem_zip if np.abs((item[1] - item[0]) / (
                            self.App.workflow_graph.nodes[node]['perf_profile'][item[1]] -
                            self.App.workflow_graph.nodes[node]['perf_profile'][item[0]])) > 1.0 / (
                                              self.App.workflow_graph.nodes[node]['BCR']) * BCRthreshold]
                else:
                ...
                available_mem_list = [item for item in
                                      np.sort(list(self.App.workflow_graph.nodes[node]['perf_profile'].keys()))]
            self.App.workflow_graph.nodes[node]['available_mem'] = available_mem_list  # Sorted list
    ...
    def PRCPG_BPBC(self, budget, BCR=False, BCRtype="RT/M", BCRthreshold=0.1):
        ...
        while (round(surplus, 4) >= 0):
            ...
            for node in cp:
                ...
                for mem in reversed(self.App.workflow_graph.nodes[node]['available_mem']):
    ...
    def get_opt_curve(self, filenameprefix, budget_list, performance_constraint_list, BCRthreshold=0.2):
        ...
        '''
        # ----- BPBC (Budget-Constrained Performance Comparison) -----
        bpbc_rows = []
        for budget in budget_list:
            ...
            rt, cost, config, iterations = self.PRCPG_BPBC(budget, BCR=True, BCRtype='RT/M', BCRthreshold=BCRthreshold)
            aRow['BCR_RT/M_RT'] = rt
            aRow['BCR_RT/M_Cost'] = cost
            aRow['BCR_RT/M_Config'] = config
            aRow['BCR_RT/M_Iterations'] = iterations

            rt, cost, config, iterations = self.PRCPG_BPBC(budget, BCR=True, BCRtype='ERT/C', BCRthreshold=BCRthreshold)
            ...
            bpbc_rows.append(aRow)

        BPBC_data = pd.DataFrame(bpbc_rows)[
        ...
        # ----- BCPC (Performance-Constrained Budget Comparison) -----
        bcpc_rows = []
        for perf_constraint in performance_constraint_list:
            ...
            bcpc_rows.append(aRow)

        BCPC_data = pd.DataFrame(bcpc_rows)[
